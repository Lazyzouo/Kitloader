...
5. Release assets must be exactly `build/libs/Kitloader-<version>-en.us.jar` and `build/libs/Kitloader-<version>-zh.cn.jar`, where `<version>` must match `version` in `build.gradle`. Do not upload custom source archives as Release assets.
