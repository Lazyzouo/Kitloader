...
4. For a new version tag, Actions creates `v<version>` and publishes only `Kitloader-en.us.jar` and `Kitloader-zh.cn.jar` with English-first, Chinese-second release notes.
...
4. 对新的版本标签，Actions 创建 `v<version>`，并只发布 `Kitloader-en.us.jar` 和 `Kitloader-zh.cn.jar`，Release Notes 先英文后中文。
