...
| --- | --- | --- |
| `Kitloader-2.0.9-en.us.jar` | `en_US` | English servers |
| `Kitloader-2.0.9-zh.cn.jar` | `zh_CN` | Simplified Chinese servers |
...
The startup banner uses a 64-column fully bordered layout with a centered version heading, bilingual Kit-management subtitle, separator, and CJK-aware aligned detail rows. The banner, startup-success message, and all update-check states use one colored console prefix and status palette.
...
| 包 | 默认 `language` | 面向服务器 |
| --- | --- | --- |
| `Kitloader-2.0.9-en.us.jar` | `en_US` | 英文服务器 |
| `Kitloader-2.0.9-zh.cn.jar` | `zh_CN` | 简体中文服务器 |
...
## 5. 更新
启动横幅采用 64 列完整边框布局，包含居中的版本标题、中英 Kit 管理副标题、分隔线及支持中日韩双列宽度的对齐详情行。横幅、启动成功消息与全部更新检查状态统一使用同一套彩色后台前缀及状态配色。
